...
std::vector<uint32_t> Huffman::count_frequencies(const std::vector<uint32_t>& string, const std::vector<Production>& productions, uint32_t size)
{
...
std::unique_ptr<Huffman::Node> Huffman::create_huffman_tree(const std::vector<uint32_t>& frequencies)
{
...
Huffman::EncodingTable Huffman::create_huffman_encode_table(const std::unique_ptr<Node>& root, uint32_t size)
{
    std::vector<std::pair<uint32_t, uint8_t>> result(size);
...
void Huffman::print_encoding_table(const EncodingTable& table)
{
